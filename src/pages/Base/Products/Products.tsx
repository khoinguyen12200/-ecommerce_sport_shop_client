    ...
    return (
        <div className='ProductsPage pt-5'>
            ...
            <div className='d-flex justify-content-between'>
                <h1 className='ml-4 mb-3'>
                    {categoryName}
                </h1>

                <div>
                    <button className='btn btn-primary mr-4' onClick={() => setShowFilter(true)}>
                        Bộ lọc
                            ...
                            <ul className="pagination justify-content-center mt-3 gap-2">
                                {
                                    Array.from(Array(pages).keys()).map((page: number) => (
                                        <li className="page-item">
                                            <Link className="page-link" to={`/products?${getLinkToPage(page + 1)}`}>{page + 1}</Link>
                                        </li>
                                    ))
                                }
                            </ul>
            ...
            <div className="galleries">
                {
                    product.productGalleries?.map((gallery: ProductGalleryInterface) => (
                        <img className='smallimg' src={getProductGalleryPath(gallery.path)} />
                    ))
                }
            </div>
            <div className='info'>
...
function Filter({show, setShow}: {show: boolean, setShow: React.Dispatch<React.SetStateAction<boolean>>}) {
    const [searchParams, setSearchParams] = useSearchParams();
    ...
    return <div className={`filter p-3 ${!show && 'hide'}`}>
        <div className="closeBtn text-right">
            <i className="btn fas fa-times" onClick={() => setShow(false)}></i>
        </div>

        <h2 className='pb-3 mb-2'>
            Sắp xếp
        </h2>
        <div className='d-flex'>
            {
                SORT_BY.map((item) => (
                    <button
                        className={`btn btn-outline-primary ${sortBy === item.value ? 'active' : ''}`}
                        onClick={() => setOneParamOfSearchParams('sortBy', item.value)}
                    >
                        {item.name}
                    </button>
                ))
        ...
        <div className='d-flex'>
            {
                ORDER_BY.map((item) => (
                    <button
                        className={`btn btn-outline-primary ${orderBy === item.value ? 'active' : ''}`}
                        onClick={() => setOneParamOfSearchParams('order', item.value)}
                    >
                        {item.name}
                    </button>
                ))
        ...
        <div className='d-flex flex-column'>
            <div className="form-floating mb-3">
                <input type="number" className="form-control"
                    onChange={(e) => setOneParamOfSearchParams('priceFrom', e.target.value)}
                />
                <label htmlFor="floatingInput">Từ</label>
            </div>
            <div className="form-floating">
                <input type="number" className="form-control"
                    onChange={(e) => setOneParamOfSearchParams('priceTo', e.target.value)}
                />
